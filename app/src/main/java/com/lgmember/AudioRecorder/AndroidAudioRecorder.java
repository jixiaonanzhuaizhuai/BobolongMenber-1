...
public class AndroidAudioRecorder {
    ...
    protected static final String EXTRA_FORMAT_BIT = "formatBit";
   ...
   // protected static final String EXTRA_KEEP_DISPLAY_ON = "keepDisplayOn";
    ...
    private int formatBit = AudioFormat.ENCODING_PCM_16BIT;
    ...
    public void record() {
        Intent intent = new Intent(activity, AudioRecorderActivity1.class);
        intent.putExtra(EXTRA_FILE_PATH, filePath);
        intent.putExtra(EXTRA_COLOR, color);
        intent.putExtra(EXTRA_SOURCE, source);
        intent.putExtra(EXTRA_CHANNEL, channel);
        intent.putExtra(EXTRA_SAMPLE_RATE, sampleRate);
        intent.putExtra(EXTRA_FORMAT_BIT, formatBit);
