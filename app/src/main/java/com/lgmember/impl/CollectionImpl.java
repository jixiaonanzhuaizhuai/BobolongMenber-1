...
public class CollectionImpl extends HttpApi {
    ...
    public void getClubCollectionList(int pageNo, int pageSize, int tag,final CollectionListBusiness.CollectionListResulHandler handler, Context context) {
        //判断没有网络应该如何处理
        if (!app.isNetWorkEnable(context)) {
            handler.onNetworkDisconnect();
        }
        ...
        MyOkHttp mMyOkhttp = new MyOkHttp(okHttpClient());
        mMyOkhttp.get()
                .url(Common.URL_CLUB_ACTIVITY_COLLECTION_LIST)
                .addParam("club", String.valueOf(0))
                .addParam("pageNo", String.valueOf(pageNo))
                .addParam("pageSize", String.valueOf(pageSize))
                .addParam("tag", String.valueOf(tag))
                .tag(this)
    ...
    public void addClubCollection(int id, final CollectionAddBusiness.CollectionResulHandler handler, Context context) {
        //判断没有网络应该如何处理
        if (!app.isNetWorkEnable(context)) {
            handler.onNetworkDisconnect();
        }
        ...
        MyOkHttp mMyOkhttp = new MyOkHttp(okHttpClient());
        mMyOkhttp.get()
                .url(Common.URL_CLUB_ADD_COLLECTION)
                .addParam("message_id", String.valueOf(id))
                .tag(this)
                .enqueue(new JsonResponseHandler() {
                    @Override
    ...
    public void deleteClubCollection(int id, final CollectionAddBusiness.CollectionResulHandler handler, Context context){
        //判断没有网络应该如何处理
        if (!app.isNetWorkEnable(context)) {
            handler.onNetworkDisconnect();
        }
        ...
        MyOkHttp mMyOkhttp = new MyOkHttp(okHttpClient());
        mMyOkhttp.get()
                .url(Common.URL_CLUB_DELETE_COLLECTION)
                .addParam("message_id",String.valueOf(id))
                .tag(this)
                .enqueue(new JsonResponseHandler() {
                    @Override
